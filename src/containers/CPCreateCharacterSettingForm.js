  ...
  render(){
    ...
    return (
      ...
      <h3 style={{textAlign:"center"}}>Where does {this.props.character.name} appear in your story? Detail it here!</h3>
        ...
        </Fragment> : <h3 style={{textAlign:"center"}}>No settings have been detailed yet for this entry. If you're interesting in creating one, you can do so <Link key={Math.random()} to={`/add-entry-info/${this.props.currentEntry.id}`}>here</Link>!</h3>}
      </div>
    )
  }
}
