...
class Profile extends Component {
    unFollowAuthor = () => {
        const deletedFollow = this.props.profileUser.follows.find((follow) => {
            return follow.following_id === this.props.currentUser.id
        })
        fetch(
            `${process.env.REACT_APP_BASE_URL}`
                .concat('/api/v1/follows/')
                .concat(`${deletedFollow.id}`),
            {
                method: 'DELETE',
            }
        )
    ...
    followingThisUser = () => {
        const followArray = this.props.profileUser.follows.filter((follow) => {
            return follow.following_id === this.props.currentUser.id
        })
    ...
    followAuthor = () => {
        fetch(`${process.env.REACT_APP_BASE_URL}`.concat('/api/v1/follows'), {
            ...
            body: JSON.stringify({
                ...
                following_id: this.props.currentUser.id,
            }),
        })
    ...
    calculatePublishedEntriesLength = () => {
        if (this.props.profileUser) {
            const publishedEntriesArray = this.props.profileUser.entries.filter(
                (entry) => {
        ...
        return (
            <Fragment>
                <div>
                    {this.props.profileUser.id === this.props.currentUser.id ? (
                        <h1>
                            Welcome back, {this.props.profileUser.username}!
                        </h1>
                    ) : (
                        <h1> {this.props.profileUser.username}'s Profile</h1>
                    )}
                    {this.props.profileUser ? (
                        <Fragment>
                            ...
                            <div style={{ textAlign: 'center' }}>
                                <img
                                    style={{
                                        border: '.25em silver solid',
                                        borderRadius: '50%',
                                        height: '8.25em',
                                        margin: '0 auto',
                                    }}
                                    src={this.props.profileUser.img_url}
                                    alt=""
                                />
                                <div
                                ...
                                >
                                    ...
                                    >
                                        <h3 style={{ textAlign: 'center' }}>
                                            {' '}
                                            {this.props.profileUser.age} Years
                                            Old
                                        </h3>
                                    </div>
                                    <div
                                    ...
                                    >
                                        <h3 style={{ textAlign: 'center' }}>
                                            ...
                                            Number of Entries:{' '}
                                            {
                                                this.props.profileUser.entries
                                                    .length
                                            }{' '}
                                            Total (
                                            {this.calculatePublishedEntriesLength()}{' '}
                                            Published)
                                        </h3>
                                    </div>
                                    <div
                                    ...
                                    >
                                        <h3>
                                            Followers:{' '}
                                            {
                                                this.props.profileUser.follows
                                                    .length
                                            }
                                        </h3>
                                        <div>
                                ...
                                <div>{this.renderProperFollowInfo()}</div>
                            </div>
                        </Fragment>
                    ) : null}
                </div>
                {this.props.profileUser.id === this.props.currentUser.id ? (
                    <Link to={`/edit-user/${this.props.currentUser.id}`}>
                        <button
                            style={{
                                position: 'relative',
                                left: '45%',
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        removeFollowFromProfileUser: (follow) => {
            dispatch({
                type: 'REMOVE_FOLLOW_FROM_PROFILE_USER',
                payload: follow,
            })
        ...
        addFollowingToCurrentUser: (following) => {
            dispatch({
                type: 'ADD_FOLLOWING_TO_CURRENT_USER',
                payload: following,
            })
        },
        removeFollowingFromCurrentUser: (following) => {
            dispatch({
                type: 'REMOVE_FOLLOWING_FROM_CURRENT_USER',
                payload: following,
            })
        },
    }
