...
class EditCharacterSettingForm extends Component {
    ...
    renderSettingRows = () => {
        if (this.props.currentEntry) {
            const settingOptionsArray = this.props.currentEntry.settings.map(
                (setting) => {
                    return (
                        <option key={Math.random()} value={setting.id}>
                            {setting.name}
                        </option>
                    )
                }
    ...
    renderCharacterRows = () => {
        if (this.props.currentEntry) {
            const characterOptionsArray =
                this.props.currentEntry.characters.map((character) => {
                    return (
    ...
    updateCharacterSetting = (charSet) => {
        fetch(
            `${process.env.REACT_APP_BASE_URL}`
                .concat('/api/v1/character_settings/')
                        ...
                        >
                            ...
                            <div style={{ position: 'absolute', left: '39%' }}>
                                {this.props.currentEntry && (
                                    <Link
                                        to={'/storyboards/'.concat(
                                            `${this.props.currentEntry.id}`
                                        )}
                                    >
                                        <button
                                            style={{
                                ...
                                <button
                                    ...
                                    onClick={(e) => {
                                        if (
                                            ...
                                            this.deleteCharacterSetting(e)
                                    }}
                                    className="ui button negative"
                                >
                                    Delete Character Setting
                                </button>
                            </div>
                        </div>
                    </Fragment>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        updateCharacterSettingInEntry: (characterSetting) => {
            dispatch({
                type: 'UPDATE_CHARACTER_SETTING_IN_ENTRY',
...
export default HOCWithAuth(
    connect(mapStateToProps, mapDispatchToProps)(EditCharacterSettingForm)
)
