...
class EntryContainer extends Component {
    ...
    filterEntries = (array) => {
        ...
        const secondFilteredArray = singleFilteredArray.filter((entry1) => {
            return entry1.genre.includes(this.state.genreSearch)
        })
        const fullFilteredArray = secondFilteredArray.filter((entry2) => {
            return entry2.published
                .toString()
                .includes(this.state.publishedSearch)
        })
    ...
    renderEntryCards = () => {
        if (this.props.currentUser && this.props.currentUser.entries) {
            const entryCardComponentArray = this.filterEntries(
                this.props.currentUser.entries
            ).map((entry) => {
