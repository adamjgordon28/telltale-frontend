...
class SPCreateCharacterSettingForm extends Component {
    ...
    renderRows = () => {
        if (this.props.currentEntry) {
            const sortedCharacters = this.props.currentEntry.characters.sort(
                function (a, b) {
            ...
            const characterOptionsArray = sortedCharacters.map((character) => {
                return (
                    <option key={Math.random()} value={character.id}>
                        {character.name}
                    </option>
                )
            })
    ...
    createCharacterSetting = (info) => {
        fetch(
            ...
            ),
            {
                ...
                body: JSON.stringify({
                    setting_id: `${this.props.setting.id}`,
            ...
            >
                ...
                >
                    <h3 style={{ textAlign: 'center' }}>
                        Which Characters appear in {this.props.setting.name} in
                        your story? Detail it here!
                    </h3>
                </div>
                {this.props.currentEntry.characters.length ? (
                    ...
                    <h3 style={{ textAlign: 'center', margin: '5%' }}>
                        No characters have been detailed yet for this entry. If
                        you're interesting in creating one, you can do so{' '}
                        <Link
                            key={Math.random()}
                            to={`/add-entry-info/${this.props.currentEntry.id}`}
                        >
                            here
                        </Link>
                        !
...
function mapDispatchToProps(dispatch) {
    return {
        addCharacterSettingToEntry: (characterSetting) => {
            dispatch({
                type: 'ADD_CHARACTER_SETTING_TO_ENTRY',
